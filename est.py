...
# 🔒 Limit CPU threads for memory safety
...
from features.page_no import add_page_number  # ✅ page numbers
from features.title import fix_title_page
# from features.numeral import process_text  # (disabled for now)
...
if uploaded_file:
    try:
        ...
        # ✅ Silent Groq API test (runs, but no UI output)
        api_key = Groq(api_key=GROQ_API_KEY)
        if api_key:
            try:
                client = Groq(api_key=api_key)
                client.chat.completions.create(
                    model="llama-3.1-8b-instant",
                    messages=[{"role": "user", "content": "Hello Groq, silent test"}],
                    max_tokens=5
                )
            except Exception:
                pass  # completely ignore Groq errors in UI

        # Step 1: Rebuild into clean APA doc
        doc = rebuild_doc_with_apa(
            old_doc,
            font_name=font_choice,
            font_size=APA_FONTS[font_choice],
        ...
        # Step 4: Rule-based heading formatting (future)
